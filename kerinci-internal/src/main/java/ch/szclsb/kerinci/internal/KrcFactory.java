...
public class KrcFactory {
    ...
    private static <T extends AbstractKrcHandle2> T construct(KrcDevice device, AbstractCreateInfo<T> createInfo, MemorySegment pCreateInfo, MemorySegment pHandle) {
    ...
    public static <T extends AbstractKrcHandle2> T create(KrcDevice device, AbstractCreateInfo<T> createInfo) {
    ...
    public static <T extends AbstractKrcHandle2> KrcArray2<T> createArray(Allocator arrayAllocator, KrcDevice device, int count, AbstractCreateInfo<T> createInfo) {
        ...
        try (var arena = Arena.ofConfined()) {
            ...
            var data = (T[]) new AbstractKrcHandle2[count];
            forEachSlice(createInfo.getLayout(), pArray, (slice, i) ->
                    data[i] = construct(device, createInfo, pCreateInfo, slice));
            return new KrcArray2<>(pArray.asReadOnly(), data);
        }
    }

    public static <T extends AbstractKrcHandle2> KrcArray2<T> createArray(Allocator arrayAllocator, KrcDevice device, List<? extends AbstractCreateInfo<T>> createInfos) {
        ...
        try (var arena = Arena.ofConfined()) {
            ...
            var data = (T[]) new AbstractKrcHandle2[createInfos.size()];
